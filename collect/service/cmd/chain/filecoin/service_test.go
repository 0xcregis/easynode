...
func TestService_GetMultiBlockByNumber(t *testing.T) {
	s, _, x := Init()
	list, _ := s.GetMultiBlockByNumber("3094030", x.WithFields(logrus.Fields{}), false)
	t.Logf("block.list:%v", len(list))
}

func TestService_GetBlockByHash(t *testing.T) {
	s, _, x := Init()
	block, txList := s.GetBlockByHash("bafy2bzacedcnnfs6tu66b5smbmjd7imo5nfee3lkmp5rh467bmnh3zzlaai66", x.WithFields(logrus.Fields{}), true)
	t.Logf("block.hash:%v, tx.list:%v", block.BlockHash, len(txList))
}

func TestService_GetTx(t *testing.T) {
	s, _, x := Init()
	tx := s.GetTx("bafy2bzacebcb3rqh24ghzr2gc4vhs7qqsncl5xad2eck2dwvnb6ub3txajzks", x.WithFields(logrus.Fields{}))
	t.Log(tx)
}

func TestService_GetReceipt(t *testing.T) {
	s, _, x := Init()
	r, err := s.GetReceipt("bafy2bzacebcb3rqh24ghzr2gc4vhs7qqsncl5xad2eck2dwvnb6ub3txajzks", x.WithFields(logrus.Fields{}))
	if err != nil {
		t.Error(err)
	} else {
		t.Logf("%+v", r)
