...
func NewMysqlService(dbConfig *config.TaskDb, chConfig map[int64]*config.ClickhouseDb, log *xlog.XLog) DbApiInterface {
...
func getNodeTaskTable() string {
	table := fmt.Sprintf("%v_%v", NodeTaskTable, time.Now().Format(DayFormat))
	return table
}
